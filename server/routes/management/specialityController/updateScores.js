...
updateScores = async (req,res)=>{
    ...
    try{
        ...
        const findData = await Speciality.findOne({where : {date : req.body.date}});
        if(findData == null){
       ...
       req.body.scoreAndId.forEach(element => {
            SpecialityEvent.update({score : element.score},{where : {militaryNumber : element.UserId}}).then(res=>{
